...
sim_scenario <- replicate(n_replic,f(x,xf,prob))



Res_m1<-rep("Randomized quantile residual (Multinomial)",each=2*n_replic)
Model<-rep(c("Null Model", "Correct Model"),each=n_replic)
...
myhnp.w<-hnp(res.mult.w.n, how.many.out = T,sim = 1000,xlab="Quantis tesricos",ylab="Resmduo quantmlico aleatorizado")

myhnp.r<-hnp(res.mult.r.n, how.many.out = T,sim = 1000,xlab="Quantis tesricos",ylab="Resmduo quantmlico aleatorizado")
